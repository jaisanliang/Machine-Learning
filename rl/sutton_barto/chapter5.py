...
class BlackjackGenerator():
	def __init__(self, start_state = None):
		...
		if start_state == None:
			self.player_cards = 2*[min(np.random.randint(1,13),10)]
			self.dealer_cards = 2*[min(np.random.randint(1,13),10)]
			# return initial state
			player_sum = sum(self.player_cards)
			if self.player_cards[0] == 1 or self.player_cards[1] == 1:
				useable_ace = 1
				player_sum += 10
			else:
				useable_ace = 0
	...
	def step(self, policy, current_state, action = None):
		...
		if action == 0:
			...
			if player_sum > 21:
				...
				if card == 1 and player_sum + 11 < 22:
					useable_ace = 1
					player_sum += 10
				return (action,(useable_ace,dealer_showing,player_sum),0)
...
def blackjack():
	...
	policies, action_values = monte_carlo_es(blackjack_states,[0,1],BlackjackGenerator)
	for player_sum in range(21,10,-1):
		for dealer_showing in range(1,11):
			sys.stdout.write(str(int(policies[(0,dealer_showing,player_sum)][0]==0))+' ')
		sys.stdout.write('\n')
	print '\n'
	for player_sum in range(21,10,-1):
		for dealer_showing in range(1,11):
			sys.stdout.write(str(int(policies[(1,dealer_showing,player_sum)][0]==0))+' ')
		sys.stdout.write('\n')
	fig = pylab.figure()
	ax = Axes3D(fig)
	y = []
	for i in range(12,22):
		for j in range(10):
			y.append(i)
	#pdb.set_trace()
	ax.scatter(y, range(1,11)*10, [values[0,i,j] for j in range(12,22) for i in range(1,11)])
	plt.show()

blackjack()

# e-soft on-policy Monte Carlo control
def monte_carlo_on_p(states, actions):
	epsilon = 0.0
	action_values = {(state,action): 0 for state in states for action in actions}
	...
	while True:
		...
		# generate random episode using policy
		while True:
			action, next_state, reward = generator(policies,current_state,current_action)
			episode_action_states.append((current_state,action))
			episode_rewards.append(reward)
			if next_state == None:
				break
		...
		for i, action_state in enumerate(episode_action_states):
			if action_state not in seen_action_states:
				seen_action_states.add(action_state)
				returns[action_state].append(episode_returns[i])
				action_values[action_state] = np.average(rewards[action_state])
		for state in policies:
			best_action = None
			best_action_value = -float('inf')
			for action in actions:
