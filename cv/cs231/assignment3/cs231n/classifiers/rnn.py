...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, cache_scores)
        if self.cell_type == 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        N,H = h0.shape
        w = self._start*np.ones((N,1)).astype(np.int32)
        for t in range(max_length):
            ...
            scores, _ = temporal_affine_forward(h0[:,None,:], W_vocab, b_vocab)
            w = np.argmax(scores,axis=2)
            captions[:,t] = np.squeeze(w)
            w = w[np.newaxis]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
