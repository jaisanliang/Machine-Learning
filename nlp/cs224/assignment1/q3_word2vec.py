...
def normalizeRows(x):
    ...
    norm = x/np.sum(x**2, axis=1, keepdims=True)**0.5
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    exp_sum = np.exp(np.dot(predicted,outputVectors.T))
    cost = -np.dot(predicted, outputVectors[target])+np.log(np.sum(exp_sum))
    gradPred = -outputVectors[target]+np.sum(outputVectors*exp_sum.reshape((-1,1)), axis=0)/np.sum(exp_sum)
    grad = predicted*np.exp(np.dot(outputVectors,predicted)).reshape((-1,1))/np.sum(exp_sum)
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    predicted = np.zeros(inputVectors[tokens[currentWord]].shape)
    for i in range(len(contextWords)):
        predicted += inputVectors[tokens[contextWords[i]]]
    ...
    cost, gradPred, grad = word2vecCostAndGradient(predicted, target, outputVectors, dataset)
    for i in range(len(contextWords)):
        gradIn[tokens[contextWords[i]]] += gradPred
    gradOut = grad
